            ...
            <?php  if(in_array( "types" ,$arraypermission )) { ?>
               ...
               <div class="table-responsive">
                  ...
                  <table class="table table-striped tablesorter display dataTable" id='empTable' style="width: 100%;">
                     <thead>
                        <tr>
                           <th class="text-center">Type Nr.</th>
...
<input type="radio" id="Parent" name="pType" value="Parent" checked>
  <label for="Parent">Parent</label>
  <input type="radio" id="sub" name="pType" value="Sub">
  <label for="sub">Sub Type</label><br>
<select class="form-select"  id="subTypes" name="parent" style="display:none;">
              <option value="">Select Sub Type....</option>
              <?php
                     $typesPayment = "select id,name from typesPayment where parent=0";
                     $typesPaymentrecord = mysqli_query($conn, $typesPayment);
                     $data = array();
                     $kudenOption='';
                     while ($typesPaymentRow = mysqli_fetch_assoc($typesPaymentrecord)) {
                        echo "<option value='".$typesPaymentRow['id']."'>".$typesPaymentRow['name']."</option>";
                     }
 ?>            
            </select>
                           </div>
         ...
         <div class="modal fade" id="addModalLabel" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
               <div class="modal-content">
                  ...
                  <div class="modal-body">
                     <form id="addForm" action="addPaymentType.php" method="POST">
                     
                        <div class="form-row">
                        <div class="form-group col-md-12">
<input type="radio" id="Parentadd" name="pTypeadd" value="Parent" checked>
  <label for="Parentadd">Parent</label>
  <input type="radio" id="subadd" name="pTypeadd" value="Sub">
  <label for="subadd">Sub Type</label><br>
<select class="form-select"  id="subTypesadd" name="parent" style="display:none;">
              <option value="">Select Sub Type....</option>
              <?php
                     $typesPayment = "select id,name from typesPayment where parent=0";
                     $typesPaymentrecord = mysqli_query($conn, $typesPayment);
 ...
 ?>            
                           ...
                           <div class="form-group col-md-12">
                              <label for="recipient-name" class="col-form-label">Choose Color:</label>
                              <!-- <input type="text" class="form-control iconpicker" name="icon" placeholder="Icon Picker" aria-label="Icone Picker"
    ...
    height: 36px;">
      ...
      <script>
         $(document).ready(function() {
            $("input[name$='pTypeadd']").click(function() {
               
               if($(this).val()=='Sub'){
               $('#subTypesadd').show();
               $("#subTypesadd").prop('required',true);
               }else{
               $('#subTypesadd').hide();
               $("#subTypesadd").prop('required',false);
               }
          }); 
            $("input[name$='pType']").click(function() {
                  ...
                  if($(this).val()=='Sub'){
                  $('#subTypes').show();
                  $("#subTypes").prop('required',true);
                  }else{
                  $('#subTypes').hide();
                  $("#subTypes").prop('required',false);
                  }
             }); 
         ...
         function viewdata(rowid) {
           $.ajax({
               url: 'viewPaymentsType.php',
               type: "POST",
               dataType: "json",
               data: {'rowId': rowid},
               success: function(response) {
                  $(".roweditid").val(response.itemType.id);
                  if(response.itemType.parent==0){
                     $("#Parent").prop("checked",true);
                     $('#subTypes').hide();
                  }else{
                     $("#sub").prop("checked",true);
                     $('#subTypes').show();
                     $("#subTypes option").filter(function() {
                 
                 return $(this).val() === response.itemType.parent;
              }).prop("selected", true);
                  }
               $("#typeName").val(response.itemType.name);
