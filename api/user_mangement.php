<?php
	...
	function access($conn){
		...
		$insertedId=0;
		$message = "";
		if ( $_GET['myApp_id'] && $_GET["kundenNr"] ) {
			 $checkExistDevice = getData2($conn, 'andriod_device', ['myApp_id' => $_GET['myApp_id'], "kunden_nr" => $_GET['kundenNr']]);
			if (count($checkExistDevice)) {
				if ($_POST['trackId']   && ( $_POST["status"] == "logged_out" )   ) {
					...
					if ($_POST["status"] == "logged_out") {
						$qry = mysqli_query($conn, "insert into user_time_mangement(user_id,logged_out) values(".$_POST['user_id'].",'".$_POST['timestamp']."')");
					}else{
						$qry = mysqli_query($conn, "insert into user_time_mangement(user_id,logged_in) values(".$_POST['user_id'].",'".$_POST['timestamp']."')");
					}
