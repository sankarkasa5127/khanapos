  ...
  width: 100%;
         ...
         .datepicker1 {
           ...
           height: 34px;
            ...
            <?php  if(in_array( "persons" ,$arraypermission )) { ?>
               <!-- <div class="row">
                  ...
                  <div class="modal-body">
                        ...
                        <div class="form-row">
                           <div class="form-group col-md-6">
                              ...
                              <input type="text" class="form-control "  value="" id="name1" name="name"  required/>
                           </div>
                           <div class="form-group col-md-6">
                              <label for="recipient-name" class="col-form-label">User Name:</label>
                              <input type="text" class="form-control "  value="" id="username1" name="username" readonly />
                           </div>
                           <div class="form-group col-md-6">
                           ...
                           <div class="form-group col-md-6">
                              <label for="recipient-name" class="col-form-label">4 Digit Pin:</label>
                              <input type="text" class="form-control "   id="digits1"   maxlength="4"  value="" name="digits"  />
                           </div>
                           ...
                           <label for="recipient-name" class="col-form-label">Role :</label>
                              <select class="form-select" name="roleid" id="roleid1" required >
                                <option value="0">Select role....</option>
                                    <?php
                                    $rolessel = "select * from roles where status=1"; 
                                    $rolequery = mysqli_query($conn, $rolessel);
                                    $rolenum = mysqli_num_rows($rolequery);
                                    $roleselect='';
                                    if($rolenum>0){
                                       while ($rolerow = mysqli_fetch_assoc($rolequery)) {
                                          echo "<option value='".$rolerow['id']."'>".$rolerow['roleName']."</option>";
                                          $roleselect.= "<option value='".$rolerow['id']."'>".$rolerow['roleName']."</option>";
                                       }
                                    }
                              ...
                              <input type="date" name='startdate' id="startdate" data-date="" class="datepicker1 form-control" data-date-format="DD.m.YYYY" value="<?php echo date('Y-m-d');?>"/>
                           </div>
                           <div class="form-group col-md-6">
                              <label for="recipient-name" class="col-form-label"> End Date:</label>
                              <input type="date" name='endDate' id="endDate" data-date="" class="datepicker1 form-control" data-date-format="DD.m.YYYY" value="<?php echo date('Y-m-d');?>"/>
                           </div>
                          
                        
                        </div>
                        
                        <button type="submit" class="btn btn-primary" id="saveuniqe">Save </button>
         ...
         <div class="modal fade" id="addModalLabel" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
               <div class="modal-content">
                  ...
                  <div class="modal-body">
                     <form id="addForm" action="aperson.php" method="POST">
                        ...
                        <div class="form-row">
                           ...
                           <div class="form-group col-md-6">
                              <label for="recipient-name" class="col-form-label">4 Digit Pin:</label>
                              <input type="number" class="form-control "   id=""   value="" name="digits"  />
                           </div>
                         
                           <div class="form-group col-md-6">
                              <label for="recipient-name" class="col-form-label">Start Date:</label>
                              <input type="date" name='startdate' data-date="" class="datepicker1 form-control" data-date-format="DD.m.YYYY" value="<?php echo date('Y-m-d');?>"/>
                             
                           </div>
                           <div class="form-group col-md-6">
                           <label for="recipient-name" class="col-form-label"> End Date:</label>
                              <input type="date" name='endDate' data-date="" class="datepicker1 form-control" data-date-format="DD.m.YYYY" value="<?php echo date('Y-m-d');?>"/>
      ...
      <script>
         ...
         function viewdata(rowid) {
           $.ajax({
               ...
               data: {'rowId': rowid},
               success: function(response) {
                  $(".roweditid").val(response.persons.id);
               $("#name1").val(response.persons.name);
              $("#username1").val(response.persons.username);
               $("#startdate").val(response.persons.startdate);
               $("#endDate").val(response.persons.endDate);
               $("#digits1").val(response.persons.digits);
                $("#startdate").attr('data-date',response.persons.startdate);
                $("#endDate").attr('data-date',response.persons.endDate);
                
               $("#status1 option").filter(function() {
                  return $(this).val() === response.persons.status;
               }).prop("selected", true);
               $("#roleid1 option").filter(function() {
                  return $(this).val() === response.persons.roleid;
               }).prop("selected", true);
