...
// Check if image file is a actual image or fake image
  if(isset($_FILES["uploadpdf"]["tmp_name"])) {
    if (!file_exists('./../../KhanaCRM/kunden/pdf/'.$invoiceid)) {
        mkdir('./../../KhanaCRM/kunden/pdf/'.$invoiceid, 0777, true);
    }
    if (!file_exists('./../../KhanaCRM/kunden/jpg/'.$invoiceid)) {
        mkdir('./../../KhanaCRM/kunden/jpg/'.$invoiceid, 0777, true);
    }
        $errors= array();
        ...
        $file_jpg = preg_replace('"\.pdf$"', '.jpg', $file_name);
            // if(is_file("KhanaCRM/kunden/pdf/".$file_name)) 
            // {
            // // Delete the given file
            // unlink("KhanaCRM/kunden/pdf/".$file_name); 
            // }
            // if(is_file("KhanaCRM/kunden/jpg/".$file_jpg)) 
            // {
            // // Delete the given file
            // unlink("KhanaCRM/kunden/jpg/".$file_jpg); 
            // }
        move_uploaded_file($file_tmp,"./../../KhanaCRM/kunden/pdf/".$invoiceid."/".$file_name);
        $pdf = './../../KhanaCRM/kunden/pdf/'.$invoiceid.'/';
        $jpg = './../../KhanaCRM/kunden/jpg/'.$invoiceid.'/';
        ...
        if (!file_exists($jpg_file)) {
           ...
           $Query = "select * from kundenDocumentsFiles WHERE rowid= ".$rowid."  ORDER BY `id` DESC";
            ...
            $i=1;
                while ($row = mysqli_fetch_assoc($Records)) {
                    $file_jpg =  $row['fileNames'];
                            ...
                            echo "<td>".$i."</td>";    
                            if(file_exists('./../../KhanaCRM/kunden/pdf/'.$row['kundenId'].'/'.$file_jpg)){
                                echo "<td class='text-center'><a data-fancybox class='fancybox' href='http://khana-gmbh.de/portal_/KhanaCRM/kunden/pdf/".$row['kundenId']."/".$file_jpg."'>".$file_jpg."</a></td>";  
                            }else{
