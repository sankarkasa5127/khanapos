...
mysqli_set_charset($conn,'utf8');
  ...
  if(isset($_FILES["uploadpdf"]["tmp_name"])) {
        ...
        $file_jpg = preg_replace('"\.pdf$"', '.jpg', $file_name);
            if(is_file("Eingangs_Rechnungen/pdf/".$file_name)) 
            {
            // Delete the given file
            unlink("Eingangs_Rechnungen/pdf/".$file_name); 
            }
            if(is_file("Eingangs_Rechnungen/jpg/".$file_jpg)) 
            {
            // Delete the given file
            unlink("Eingangs_Rechnungen/jpg/".$file_jpg); 
            }
        move_uploaded_file($file_tmp,"Eingangs_Rechnungen/pdf/".$file_name);
        $pdf = 'Eingangs_Rechnungen/pdf/';
        $jpg = 'Eingangs_Rechnungen/jpg/';
     ...
     if(isset($_FILES["worddocument"]["tmp_name"])) {
       ...
       // $file_jpg = preg_replace('"\.pdf$"', '.jpg', $file_name);
            if(is_file("Eingangs_Rechnungen/word/".$file_name)) 
            {
            // Delete the given file
            unlink("Eingangs_Rechnungen/word/".$file_name); 
            }
        move_uploaded_file($file_tmp,"Eingangs_Rechnungen/word/".$file_name);
     }
     ...
     if(isset($_FILES["uploadjpg"]["tmp_name"])) {
       ...
       // $file_ext=strtolower(end(explode('.',$_FILES['worddoc']['name'])));
       // $file_jpg = preg_replace('"\.pdf$"', '.jpg', $file_name);
            if(is_file("Eingangs_Rechnungen/jpg/".$file_name)) 
            {
            // Delete the given file
            unlink("Eingangs_Rechnungen/jpg/".$file_name); 
            }
        move_uploaded_file($file_tmp,"Eingangs_Rechnungen/jpg/".$file_name);
     }
