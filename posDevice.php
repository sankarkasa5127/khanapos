...
<style>
    #editdevicemodel #datum{
        ...
        <div class="col-sm-9 offset-sm-3 col-md-10 offset-md-2 main">
            <?php 
                $totalDevices = mysqli_query($conn, "select count(*) as total_devices from andriod_device");
                $totalApps = mysqli_query($conn, "select COUNT(*) as total_apps FROM `andriod_device` WHERE myApp_id != 'null'");
                $totalActive = mysqli_query($conn, "select COUNT(*) as total_active FROM `andriod_device` WHERE status = 1");
                $totalInactive = mysqli_query($conn, "select COUNT(*) as total_inactive FROM `andriod_device` WHERE status != 1");
                $totalInactive = mysqli_query($conn, "select COUNT(*) as total_inactive FROM `andriod_device` WHERE status != 1");
                $newDevices = mysqli_query($conn, "select COUNT(*) as new_devices FROM `andriod_device` WHERE created_at BETWEEN '".date('Y-m-d 00:00:00')."' AND '".date('Y-m-d 23:23:59')."'");               
            ?>
            <div class="row">
                  <div class="col-lg-3 col-md-3">
                     <div class="price-box">
                        ...
                        <div class="contant-box">
                           <h3 class="main-title">Total Devices</h3>
                           <p class="tprice"><?= mysqli_fetch_assoc($totalDevices)['total_devices'] ?>&nbsp;</p>
                        </div>
                     </div>
                  </div> 
                  <div class="col-lg-3 col-md-3">
                     <div class="price-box">
                        <i class="fas fa-inbox icon1"></i>
                        <div class="contant-box">
                           <h3 class="main-title">Total Apps</h3>
                           <p class="tprice"><?= mysqli_fetch_assoc($totalApps)['total_apps'] ?>&nbsp;</p>
                        </div>
                     </div>
                  </div>
                  <div class="col-lg-3 col-md-3">
                     <div class="price-box">
                        <i class="fas fa-inbox icon1"></i>
                        <div class="contant-box">
                           <h3 class="main-title">Total Active/Inactive</h3>
                           <p class="tprice"><?= mysqli_fetch_assoc($totalActive)['total_active'].' / '.mysqli_fetch_assoc($totalInactive)['total_inactive'] ?>&nbsp;</p>
                        </div>
                     </div>
                  </div>
                  <div class="col-lg-3 col-md-3">
                     <div class="price-box">
                        ...
                        <div class="contant-box">
                           ...
                           <p class="tprice"><?= mysqli_fetch_assoc($newDevices)["new_devices"] ?>&nbsp;</p>
                        </div>
                     </div>
                  </div>
               </div>
            ...
            <table class="table table-striped tablesorter display dataTable" id="table-1">
              <thead>
                <tr>
                  ...
                  <th style="width:100px;">My Device ID</th>
                  <th style="width:100px;">My App ID</th>
                  <th style="width:100px;">KD ID</th>
                 <th style='width:100px;'>Server IP</th>
                 <th style='width:100px;'>Port</th>
                 <th style='width:100px;'>Status</th>
                 <th style='width:100px;'>Device On/Off</th>
         ...
         <div class="modal fade" id="editdevicemodel" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
               <div class="modal-content">
                  ...
                  <div class="modal-body">
                     <form id="editForm" action="device.php?type=update" method="POST">
                        <div class="form-row">
                        ...
                        <div class="form-row">
                            <input type="hidden" name="id" class="roweditid" value="" />
                            <div class="form-group col-md-6">
                              <label for="recipient-name-nuksaan" class="col-form-label">Server IP :</label>
                              <select class="form-control" id="server_id" name="server_id" style="height: 3.4rem;">
                                    <option selected disabled>Select Server ID</option>
                                    <?php
                                        $serverQuery = "select * from andriod_device where server_id = 0";
                                        $serverRecord = mysqli_query($conn, $serverQuery);
                                        while ($serverRow = mysqli_fetch_assoc($serverRecord)) {
                                            echo "<option value='".$serverRow['device_id']."'>".$serverRow['device_id']."</option>";
                                        }
                                    ?>
                              </select>
                           </div>
                        ...
                        <div class="form-row">
                           ...
                           <div class="form-group col-md-6">
                              <label for="recipient-name-nuksaan" class="col-form-label">Server Port (if required) :</label>
                        ...
                        <div class="form-group col-md-6">
                              ...
                              <select class="form-control" id="typ" name="typ" style="height: 3.5rem;">
                                    <?php
                                        $deviceTypeQuery = "select * from device_type";
                                        $deviceTypeRecord = mysqli_query($conn, $deviceTypeQuery);
                                        while ($deviceTypeRow = mysqli_fetch_assoc($deviceTypeRecord)) {
                                            echo "<option value='".$deviceTypeRow['id']."'>".$deviceTypeRow['name']."</option>";
                                        }
                                    ?>
                              </select>
                           </div>
...
function deleteRecord(id){
  ...
  function view(device_id) {
           $.ajax({
               url: 'device.php?id='+device_id+'&type=getData',
               // url: 'device.php?device_id='+device_id+'&myDevice_id='+myDevice_id,
               type: "POST",
               dataType: "json",
               data: {},
  ...
  <script>
    ...
    const pusher = new Pusher('86563640c9a1a9a32002', {
        ...
        */
        ...
        const channel2 = pusher.subscribe("device-channel");
        channel2.bind("online-event", function(data) { 
            if(data)
                data = JSON.parse(data);
            changeStatus(data.id, data.status, data.clientMessage);
        });

        const channel3 = pusher.subscribe("device-online");
        channel3.bind("offline-event", function(data) { 
            if(data)
                data = JSON.parse(data);
            changeStatus(data.id, data.status, data.clientMessage);
        });
        ...
        function changeStatus(id,statusMode,message){
            let status = (statusMode == 'offline') ? 0 : 1;
            let type = (statusMode == 'offline') ? 'danger' : 'success';
            $.ajax({
               url:"/changeDeviceStatus.php",
               method: "post",
               data: "device_status="+status+"&id="+id,
               success: function(response){
                    $('#table-1').DataTable().ajax.reload();
               }
            });
   ...
   $('#table-1').DataTable({
      ...
      'columns': [
        ...
        $(document).ready(function(){
            setInterval(()=>{
                 $.ajax({
                    ...
                    type: "get",
                    success: function(response){
                       console.log("pong sent at "+ Math.floor(Date.now() / 1000));
                       setTimeout(() => {
                        getResponse();
                       }, 1000);
                    }
         ...
         function getResponse(){
            $.ajax({
                url: "check_offline.php",
                success:function(response){
                    if(response){
						console.log(response)
                        data = JSON.parse(response);
                        changeMultiStatus(data.devices,0);
                        changeMultiStatus(Object.values(data.online),1);
                    }
        ...
        function changeMultiStatus(id,status){
            $.ajax({
                   url:"/changeDeviceStatus.php?type=multiple",
                   method: "post",
                   data: "device_status="+status+"&id="+id,
                   success: function(response){
                   }
