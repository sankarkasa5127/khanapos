    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-sm-3 col-md-2 sidebar">
		  ...
		  <ul class="nav nav-sidebar">
            <li class="active"><a href="Kassen_protokoll.php">Kassen Protokoll  <span class="sr-only">(current)</span></a></li>  
...
//$sql = "SELECT MAX(`id`),`id`,`anydesk`,`terminal`,`steuerid`,`kunden`,`note`,`street`,`zipcode`,`stadt`,`datum`,`updated` FROM `kassen_protokoll` group by `anydesk` ORDER BY `id` DESC ";
$sql = "SELECT * FROM `kassen_protokoll` ORDER BY `id` DESC ";
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
       ...
       $version="";
		...
		<td style='width:100px;'> ".$row["anydesk"]." </td>
		<td style='width:100px;'> ".$row["terminal"]." </td>
		 
		<td style='width:100px;'> ".$row["kunden"]." </td>
		<td style='width:100px;'> ".$row["street"]."<br>".$row["zipcode"]." ".$row["stadt"]." </td>		 
		<td style='width:100px;'>".$tseText."</td>
		<td style='width:100px;'>".$tierText."</td>
		<td style='width:100px;'> ".$version." </td>
		<td style='width:100px;'> ".$row["datum"]." </td>";	?>
		<td style='width:100px;'><input type="button" class="button" onclick="deleteRecord(<?php echo $row["id"]; ?>)" value="Delete"></td>
	<?php echo	"</tr>";		 
    }
} else {
    //echo "0 results";
