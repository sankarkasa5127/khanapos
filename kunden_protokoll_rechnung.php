...
$sql = "SELECT * FROM restaurants ORDER BY lpad(id, 10, 0) DESC";
...
//		if($row["Status"]==1) { $status = '<img src="img/green_small.png"/>'; } elseif($row["Status"]==2) { $status = '<img src="img/yellow_small.png"/>'; } elseif($row["Status"]==3) { $status = '<img src="img/gray_small.png"/>'; } else {$status = '<img src="img/red_small.png"/>';}
	  ...
	  echo "<tr>		
		...
		<td> ".$row["restaurant_str"]." ".$row["restaurant_str_nr"]." ".$row["restaurant_plz"]." </td>
		<td> ".$row["restaurant_ort"]." </td>
        <td><form id='update_form".$row["id"]."' method='POST'><input type='hidden' name='posId' value='".$row["id"]."'> <input value='".$row["rechnung_nr"]."' type='text' name='rechnung_nr' id='rechnung_nr".$row["id"]."'><input onClick='update_rechnungNr(".$row["id"].")' type='button' class='button' value='Edit'></td>					
...
?>
 ...
 <script>			
	function update_rechnungNr(id){
		var aa = $("#rechnung_nr"+id).val();
		
    $.ajax('update_RechnungNr.php', {
